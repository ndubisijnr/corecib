...
export default {
  ...
  methods: {
    ...
    rowClass(item, type) {
      ...
      if (item.status != null)
        if (
          item.status.toLowerCase() === "invalid account" &&
          item.status != null
        )
          return "table-danger";
        else if (
          item.status.toLowerCase() === "account validated" &&
          item.beneficiaryname.toLowerCase() === item.nename.toLowerCase()
        )
          return "table-success";
        else if (item.status.toLowerCase() === "account validated")
          return "table-primary";
        else if (item.status.toLowerCase().trim() === "no response from server")
